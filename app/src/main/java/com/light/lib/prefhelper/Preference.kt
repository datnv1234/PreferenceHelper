...
class Preference {
    ...
    companion object {
        ...
        private fun getInstance(): Preference? {
            requireNotNull(instance) { "Pref must be call initHelper on Application before using." }
            return instance
        }

        fun initHelper(context: Context): Preference? {
            if (instance == null) instance = Preference(context)
            return instance
        }

        fun initHelper(context: Context, sharePreferencesName: String): Preference? {
            if (instance == null) instance = Preference(context, sharePreferencesName)
            return instance
        }
